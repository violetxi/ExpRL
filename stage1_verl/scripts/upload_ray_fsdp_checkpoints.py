...
def process_checkpoint(local_dir: str, output_dir: str) -> None:
    ...
    if not os.path.exists(hf_path):
        ...
        if os.path.exists(parent_hf_path):
            ...
            raise FileNotFoundError(f"HuggingFace config not found in {hf_path} or parent directory")
...
def get_branch_name(checkpoint_path: str) -> str:
    ...
    return os.path.normpath(checkpoint_path).split(os.sep)[-2]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--main_checkpoint', default=None, type=str,
                        help="Path to the checkpoint that should be set as the main branch. "
                             "If unset, the main branch is left untouched and every checkpoint "
    ...
    checkpoint_paths = [os.path.join(args.parent_dir, checkpoint_dir, 'actor') 
    ...
    if args.main_checkpoint is None:
        print("No --main_checkpoint specified; main branch will not be touched.")
    else:
        if not os.path.exists(args.main_checkpoint):
