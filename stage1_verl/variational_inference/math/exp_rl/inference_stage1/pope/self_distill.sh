#!/bin/bash
#SBATCH --job-name=self_distill_stage1
...
model.path=${HF_NAMESPACE:-violetxi}/exp_stage1_qwen3-4b_self_distill \
model.revision=main \
data.path=$test_path \
...
data.output_path=${HF_NAMESPACE:-violetxi}/pope_hard_stage1_self_distill \
data.chunk_output_dir=./chunks_pope_hard_stage1_self_distill \
