#!/bin/bash
#SBATCH --job-name=olympiad_physics_qwen8b_grpo_stage1
...
###### OlympiadBench-OE Physics (OE_TO_physics_en_COMP) — GRPO (exp_rl_all_domains_stage1_qwen8b_grpo)
...
model.path=${HF_NAMESPACE:-violetxi}/exp_rl_all_domains_stage1_qwen8b_grpo \
model.revision=main \
data.path=$test_path \
...
data.output_path=${HF_NAMESPACE:-violetxi}/olympiad_physics_stage1_qwen8b_grpo \
data.chunk_output_dir=./chunks_olympiad_physics_stage1_qwen8b_grpo \
