#!/bin/bash
#SBATCH --job-name=dense_outcome_qwen8b
...
# NOTE: OE physics rows use the Gemini Flash Lite LLM judge for reward
# (verl/utils/reward_score/sciknoweval_judge.py). Make sure GEMINI_API_KEY is
# exported in the env if you want OE rewards to fire (MCQ uses the deterministic
# letter-match path, no API key needed).
...
local_dir=checkpoints/dense_outcome_qwen8b_all_domains/
...
python3 -m verl.trainer.main_rl_genrm \
    ...
    +custom_reward_function.prompt_template_paths.math=verl/utils/prompt_templates/math_likelihood_judge.py \
    ...
    trainer.experiment_name='dense_outcome' \
