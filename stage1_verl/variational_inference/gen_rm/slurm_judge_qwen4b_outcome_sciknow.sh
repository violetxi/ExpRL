#!/bin/bash
#SBATCH --job-name=judge_qwen4b_sciknow_h100
...
# Usage:
#   sbatch slurm_judge_qwen4b_outcome_sciknow.sh [mcq|oe|all]
# Default is "all" — runs MCQ then OE in the same allocation (vLLM loads twice
# but you only burn one job slot). Pass "mcq" or "oe" to do a single pass.
...
# (data_path, output_path) pairs that DATASET_KEY selects.
...
declare -A SCIKNOW_OUTPUT=(
    [mcq]="${HF_NAMESPACE:-violetxi}/Qwen4b-Instruct-Judge-sciknoweval-mcq-l3-loose-qwen8b-multi-ref"
    [oe]="${HF_NAMESPACE:-violetxi}/Qwen4b-Instruct-Judge-sciknoweval-oe-v1-qwen8b-multi-ref"
)
