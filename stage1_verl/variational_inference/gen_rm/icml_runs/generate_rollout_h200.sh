...
test_path=data/process_reward/omni-rule-l7-above-gemini-pro-filtered_qwen3-4b-instruct-2507-steps/train.parquet
python3 -m verl.trainer.main_generation \
--config-path=config --config-name=generation \
model.path=qwen/Qwen3-4B-Instruct-2507 \
model.revision=main \
...
data.use_chat_template=True \
data.enable_thinking=False \
data.output_path=${HF_NAMESPACE:-violetxi}/process_rollout-omni-rule-l7-above-gemini-pro-filtered \
rollout.prompt_length=16384 \
rollout.response_length=16384 \
rollout.temperature=0.8 \
rollout.tensor_model_parallel_size=1 \
