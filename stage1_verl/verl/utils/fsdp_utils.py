...
def _get_total_norm_compat(tensors, norm_type=2.0, error_if_nonfinite=False, foreach=None):
    """Backport of torch>=2.6 torch.nn.utils.clip_grad._get_total_norm for torch 2.5."""
    ...
    if error_if_nonfinite and torch.logical_or(total_norm.isnan(), total_norm.isinf()):
        raise RuntimeError(
            ...
            "non-finite, so it cannot be clipped. To disable this error and scale the "
            "gradients by the non-finite norm anyway, set `error_if_nonfinite=False`"
        )
    return total_norm


def _clip_grads_with_norm_compat(parameters, max_norm, total_norm, foreach=None):
    """Backport of torch>=2.6 torch.nn.utils.clip_grad._clip_grads_with_norm_ for torch 2.5."""
    from torch.utils._foreach_utils import (
        _device_has_foreach_support,
        _group_tensors_by_device_and_dtype,
        _has_foreach_support,
...
def fsdp2_clip_grad_norm_(parameters, max_norm, norm_type=2.0, error_if_nonfinite=False, foreach=None):
    ...
    try:
        ...
        # torch < 2.6 didn't expose these helpers; use the backports above.
        _get_total_norm = _get_total_norm_compat
        _clip_grads_with_norm_ = _clip_grads_with_norm_compat
