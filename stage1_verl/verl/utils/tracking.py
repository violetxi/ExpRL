...
class ValidationGenerationsLogger:
    ...
    def _log_generations_to_wandb(self, samples, step, wandb):
        ...
        def _sample_columns(sample_idx, sample):
            base_columns = ["input", "output", "score", "judge_output", "judge_reasoning"]
            return [f"{base_columns[i] if i < len(base_columns) else f'extra_{i + 1}'}_{sample_idx}" for i in range(len(sample))]

        # Create column names for all samples
        columns = ["step"] + sum([_sample_columns(i + 1, sample) for i, sample in enumerate(samples)], [])
    ...
    def log_generations_to_swanlab(self, samples, step):
        ...
        # Create column names
        base_headers = ["input", "output", "score", "judge_output", "judge_reasoning"]
        max_sample_len = max((len(sample) for sample in samples), default=3)
        headers = ["step"] + [
            base_headers[i] if i < len(base_headers) else f"extra_{i + 1}" for i in range(max_sample_len)
        ]
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                for sample in samples:
                    ...
                    if len(sample) >= 5:
                        data["judge_output"] = sample[3]
                        data["judge_reasoning"] = sample[4]
        ...
        table = [
            {
                ...
                "output": sample[1],
                "score": sample[2],
                **({"judge_output": sample[3], "judge_reasoning": sample[4]} if len(sample) >= 5 else {}),
            }
            for sample in samples
        ]
    ...
    def log_generations_to_tensorboard(self, samples, step):
        ...
        for i, sample in enumerate(samples):
            ...
            if len(sample) >= 3:
                ...
                if len(sample) >= 5:
                    text_content += f"**Judge Output:** {sample[3]}\n\n"
                    text_content += f"**Judge Reasoning:** {sample[4]}\n\n"
            else:
