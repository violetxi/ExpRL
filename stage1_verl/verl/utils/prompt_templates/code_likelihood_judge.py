...
The user message contains three distinctly-labelled sections:
...
* **REFERENCE** — a known-correct gold solution. It may contain a written explanation of the intended approach followed by reference code. Use it only to understand what "correct" looks like.

Your task is to judge whether the **submitted program** (from POLICY SUBMISSION) is functionally equivalent to the reference — i.e. would it pass the same test cases.
...
### Critical: do not cross-quote and do not anchor on surface similarity
* **Every checklist quote you cite must come from inside the POLICY SUBMISSION markers.** Never quote from REFERENCE as evidence about the submission. If you cannot find a quote inside POLICY SUBMISSION that supports a verdict, the verdict is No.
* Different variable names, refactored control flow, **different time/space complexity classes**, different primitives (e.g. string vs math, sort vs single-pass, brute force vs optimized), or **a different but provably-equivalent algorithm** all count as correct. "Different from the reference" is not evidence of wrongness.
* Conversely, code that *looks* similar to the reference can still be wrong. The most common failure mode is subtle: a condition flipped, a wrong modulus, an off-by-one in loop bounds, a missing branch, a `<` vs `<=`, an index used before it's set. Look for these explicitly rather than reading the submission as "basically the reference."
* If the POLICY SUBMISSION analysis claims to do X but the submitted code does Y, the code wins.
...
### Step 1 — Checklist
Be strict: "partially" or "unclear" counts as **No**. For each Yes verdict, your justification must reference a specific line/expression from the **POLICY SUBMISSION's submitted code** (not its analysis, not the REFERENCE).
...
2. **Algorithmic correctness (NOT similarity to the reference).** Would the submitted algorithm produce a correct output for valid inputs? Mark **No only if you can name a specific valid input on which the submitted code would produce a wrong output** (different from what the problem requires). Do **not** mark No because the submission uses a different strategy, different data structures, higher asymptotic complexity, or different primitives than the reference. If you cannot name a specific failing input, the verdict is Yes.
...
4. **Edge cases & constraints.** Does the submitted code handle the edge cases that matter for this problem (empty input, single element, boundary values, ties, overflow / modulus, off-by-one) wherever the reference handles them? Name the specific edge case and quote the handling line from the submitted code, or note its absence.
...
### Step 2 — Score
Apply in order:
* **5 (Functionally Equivalent):** All five checks are Yes. The submitted code would pass the same tests as the reference.
* **4 (Likely Correct):** Checks 1, 2, 3, and 5 are Yes, and check 4 is No only on a minor edge case the reference handles but that is unlikely to be tested heavily.
...
2. Algorithmic correctness:
   Analysis: [Describe what the submitted algorithm does. Then: can you name a specific valid input on which it would produce a wrong output? If yes, state it explicitly. If no, the verdict is Yes regardless of structural difference from the reference.]
   Verdict: [Yes/No]
...
==================== REFERENCE (known correct) ==========================
The text between these markers is the gold solution. It may include a
written explanation of the intended approach followed by the reference
code. Use it to understand what "correct" looks like; do NOT quote from
this section as evidence about the submission.
--------------------------------------------------------------------------
{reference_solution}
--------------------------------------------------------------------------
========================== END REFERENCE ================================
