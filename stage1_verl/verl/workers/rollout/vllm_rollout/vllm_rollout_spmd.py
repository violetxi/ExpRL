...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        max_num_seqs = self.config.get("max_num_seqs", 1024)
        ...
        for k in config.keys():
            if hasattr(SamplingParams(), str(k)) and k != "seed":                
                if k == "stop_token_ids":    # convert to python list
                    kwargs[k] = OmegaConf.to_container(config.stop_token_ids)                
                elif k == "stop":
                    kwargs[k] = OmegaConf.to_container(config.stop)
                else:
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if not do_sample:
            ...
            kwargs = {
                ...
                "stop_token_ids": [], # no stop token in validation
                "stop": [], # no stop token in validation
        ...
        with self.update_sampling_params(**kwargs):
            print(f"Generating sequences with sampling params: {self.sampling_params}")
