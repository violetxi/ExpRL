        ...
        def loss_func(output, data, meta_info):
            ...
            if not forward_only:
                ...
                if isinstance(loss_output, dict):
                    pg_loss = loss_output.get("pg_loss", torch.tensor(0.0, device=device))
                    pg_clipfrac = loss_output.get("pg_clipfrac", torch.tensor(0.0, device=device))
                    ppo_kl = loss_output.get("ppo_kl", torch.tensor(0.0, device=device))
                    pg_clipfrac_lower = loss_output.get("pg_clipfrac_lower", torch.tensor(0.0, device=device))
                else:
