...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        self._is_dpo = self.config.get("is_dpo", False)
    ...
    def _build_model_optimizer(
        ...
        self.tokenizer = hf_tokenizer(local_path, trust_remote_code=trust_remote_code, revision=revision)
        self.processor = hf_processor(local_path, trust_remote_code=trust_remote_code, revision=revision)
        ...
        actor_model_config = AutoConfig.from_pretrained(
            local_path, trust_remote_code=trust_remote_code, attn_implementation="flash_attention_2", revision=revision
        )
    ...
    def init_model(self):
        if self._is_dpo:
            from verl.workers.actor import DataParallelDPOActor
            actor_cls = DataParallelDPOActor
        else:
            policy_loss = self.config.actor.get("policy_loss", None)
            if policy_loss is not None:
                loss_mode = self.config.actor.policy_loss.get("loss_mode", "vanilla")
            else:
                loss_mode = self.config.actor.get("loss_mode", "vanilla")

            if loss_mode == "vi_vanilla":
                from verl.workers.actor import DataParallelVIActor
                actor_cls = DataParallelVIActor
            elif loss_mode == "distill":
                from verl.workers.actor import DataParallelDistillActor
                actor_cls = DataParallelDistillActor
            else:
        ...
        if self._is_ref:
            ...
            self.ref_policy = actor_cls(config=self.config.ref, actor_module=self.ref_module_fsdp)
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            if self._is_dpo:
                chosen_log_probs, rejected_log_probs, _, _ = \
                    self.ref_policy.compute_log_prob(data=data, calculate_entropy=False)
                output = DataProto.from_dict(tensors={"ref_chosen_log_probs": chosen_log_probs, "ref_rejected_log_probs": rejected_log_probs})
            else:
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def sleep_vllm(self) -> None:
        """Release vLLM memory (KV cache and optionally weights) to free GPU memory.
        
        This is useful when another model (e.g., GenRM) needs to use GPU memory.
        Call wake_vllm() before using generate_sequences() again.
        """
        if self._is_rollout and hasattr(self, 'rollout_sharding_manager'):
            if hasattr(self.rollout_sharding_manager, 'inference_engine'):
                self.rollout_sharding_manager.inference_engine.sleep(level=1)
                log_gpu_memory_usage("After actor vLLM sleep", logger=logger)
                get_torch_device().empty_cache()

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def wake_vllm(self) -> None:
        """Wake up vLLM and reallocate KV cache memory.
        
        This is called automatically by generate_sequences() via the sharding manager,
        but can be called explicitly if needed.
        """
        if self._is_rollout and hasattr(self, 'rollout_sharding_manager'):
            if hasattr(self.rollout_sharding_manager, 'inference_engine'):
                self.rollout_sharding_manager.inference_engine.wake_up()
                log_gpu_memory_usage("After actor vLLM wake_up", logger=logger)


class GenRMWorker(Worker, DistProfilerExtension):
    """
    GenRM Worker for reward scoring.
    
    This worker is a separate model instance used for generating reward scores.
    It supports CPU offloading via FSDP for memory efficiency and uses vLLM for
    efficient generation. Unlike ActorRolloutRefWorker, this is inference-only
    (no training) and the weights are frozen.
    """

    def __init__(self, config: DictConfig, role: str = "genrm", **kwargs):
        Worker.__init__(self)
        self.config = config
        self.role = role
        self.profile_option = kwargs.get("profile_option", None)
        ...
        # Build device mesh for FSDP
        world_size = torch.distributed.get_world_size()
        fsdp_size = self.config.fsdp_config.get("fsdp_size", -1)
        self.device_mesh = create_device_mesh(world_size=world_size, fsdp_size=fsdp_size)

        # GenRM is always rollout-only (inference), no actor/ref roles
        ...
        # Enable CPU offload for memory efficiency (GenRM can be offloaded when not scoring)
        self._is_offload_param = self.config.fsdp_config.get("param_offload", True)

    def _build_model(self, model_path: str, trust_remote_code: bool = False):
        """Build the FSDP-wrapped model for GenRM."""
        ...
        log_gpu_memory_usage("Before init GenRM model from HF AutoModel", logger=logger)
        ...
        # Override config with tokenizer special tokens
        override_config_kwargs = {
            "bos_token_id": self.tokenizer.bos_token_id,
            "eos_token_id": self.tokenizer.eos_token_id,
            "pad_token_id": self.tokenizer.pad_token_id,
        ...
        if self.rank == 0:
            print(f"GenRM model config: {model_config}")
        ...
        log_gpu_memory_usage("After loading GenRM model", logger=logger)

        # Wrap with FSDP for memory management
        fsdp_config = omega_conf_to_dataclass(self.config.fsdp_config)
        auto_wrap_policy = get_fsdp_wrap_policy(model, fsdp_config.wrap_policy)
        sharding_strategy = get_sharding_strategy(self.device_mesh)

        if self.config.get("strategy", "fsdp") == "fsdp":
            model_fsdp = FSDP(
                model,
                param_init_fn=init_fn,
        ...
        elif self.config.get("strategy", "fsdp") == "fsdp2":
            assert CPUOffloadPolicy is not None, "PyTorch >= 2.4 required for FSDP2"
            cpu_offload = CPUOffloadPolicy(pin_memory=True) if self._is_offload_param else None
            fsdp_kwargs = {
            ...
            fsdp2_load_full_state_dict(model, full_state, self.device_mesh, cpu_offload)
            model_fsdp = model
        else:
            raise NotImplementedError(f"Unknown strategy: {self.config.get('strategy', 'fsdp')}")

        log_gpu_memory_usage("After FSDP wrapping GenRM model", logger=logger)

        return model_fsdp

    def _build_rollout(self, trust_remote_code: bool = False):
        """Build the vLLM rollout engine for generation."""
        from torch.distributed.device_mesh import init_device_mesh

        infer_tp = self.config.rollout.tensor_model_parallel_size
        dp = self.world_size // infer_tp
        assert self.world_size % infer_tp == 0, (
            f"GenRM world_size: {self.world_size} is not divisible by infer_tp: {infer_tp}"
        )
        rollout_device_mesh = init_device_mesh(
            device_name, mesh_shape=(dp, infer_tp), mesh_dim_names=["dp", "infer_tp"]
        )
        ...
        if rollout_name == "vllm":
            ...
            log_gpu_memory_usage(f"Before building GenRM {rollout_name} rollout", logger=logger)
            local_path = copy_to_local(self.config.model.path, use_shm=self.config.model.get("use_shm", False))
            ...
            log_gpu_memory_usage(f"After building GenRM {rollout_name} rollout", logger=logger)
            full_params = torch.distributed.get_world_size() == 1
            rollout_sharding_manager = FSDPVLLMShardingManager(
            ...
            log_gpu_memory_usage("After building GenRM sharding manager", logger=logger)
            ...
            rollout_sharding_manager = FSDPSGLangShardingManager(
                ...
                device_mesh=rollout_device_mesh,
                offload_param=self._is_offload_param,
                multi_stage_wake_up=self.config.rollout.get("multi_stage_wake_up", False),
            )
            log_gpu_memory_usage("After building GenRM sharding manager", logger=logger)
            ...
            rollout = HFRollout(module=self.model_fsdp, config=self.config.rollout)
            rollout_sharding_manager = BaseShardingManager()

        else:
            raise NotImplementedError(f"GenRM rollout name '{rollout_name}' is not supported")

        return rollout, rollout_sharding_manager

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def init_model(self):
        """Initialize the GenRM model and rollout engine."""
        import_external_libs(self.config.model.get("external_lib", None))

        trust_remote_code = self.config.model.get("trust_remote_code", False)
        local_path = copy_to_local(self.config.model.path, use_shm=self.config.model.get("use_shm", False))

        # Build FSDP-wrapped model
        self.model_fsdp = self._build_model(local_path, trust_remote_code=trust_remote_code)

        # Build rollout engine
        self.rollout, self.rollout_sharding_manager = self._build_rollout(trust_remote_code=trust_remote_code)

        log_gpu_memory_usage("GenRM init_model complete", logger=logger)

    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    @DistProfiler.annotate(color="green", role="genrm_generate")
    def generate_sequences(self, prompts: DataProto):
        """Generate sequences using the GenRM model."""
        prompts = prompts.to(get_device_id())
        ...
        timing_generate = {}
        with self.rollout_sharding_manager:
            log_gpu_memory_usage("After entering GenRM rollout sharding manager", logger=logger)
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def start_profile(self, **kwargs) -> None:
        """Start profiling for the current rank."""
        self.profiler.start(**kwargs)

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def stop_profile(self) -> None:
        """Stop profiling for the current rank."""
        self.profiler.stop()
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if config.model.path == 'Nexusflow/Athene-RM-8B':    # special case for Athena RM
                from verl.workers.reward_manager import AtheneForSequenceClassification
                reward_module = AtheneForSequenceClassification.from_pretrained('Nexusflow/Athene-RM-8B',
                                                                                torch_dtype=torch.bfloat16,
                                                                                attn_implementation='flash_attention_2')
            else:
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if self.config.model.path == 'Nexusflow/Athene-RM-8B':
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            if isinstance(raw_prompt, str):
                chat: list = [{"role": "user", "content": raw_prompt}]
            else:
                chat: list = raw_prompt

            assert isinstance(chat, list), f"chat must be a list, got {type(chat)}"
            # extract response
            ...
            # if the model decides to think, we need to remove the think part when it's 
            # finished make the actual response nothing if the thinking is not finished
            ...
            prompt_with_chat_template = target_tokenizer.apply_chat_template(
             ...
             # add cls token for Athena RM
            if self.config.model.path == 'Nexusflow/Athene-RM-8B':
                prompt_with_chat_template = prompt_with_chat_template + self.tokenizer.cls_token
                
            if "<think>" in prompt_with_chat_template or "</think>" in prompt_with_chat_template:
                print(f"WARNING: <think> or </think> should not be seen by the reward model! chat: {prompt_with_chat_template}")
                # raise ValueError("Error: <think> or </think> should not be seen by the reward model!")
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if self.config.expand_to_token_level:
