...
class AdvantageEstimator(str, Enum):
    ...
    GPG = "gpg"
    GPPO = "group_process"
    GPR_E = "group_process_end"
    PR_BN = "process_bn"
    PR_DELTA = "delta_process"
    PR_BASIC = "basic_process"
    BASIC = "basic"
...
@register_adv_est(AdvantageEstimator.GPR_E)  # or simply: @register_adv_est("grpo")
def compute_gpr_e_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    """
    Compute advantage for Group Process Reward End (GPR_E), where the last step reward is used as the reward for the last step.
    Subtract the last step reward from the token level rewards as baseline.
    """
    scores = token_level_rewards.clone()
    with torch.no_grad():
        bsz = scores.shape[0]
        for i in range(bsz):
            if last_step_reward[i] is not None:    # when score is None, we can ignore cuz no loss is computed
                scores[i] = scores[i] - last_step_reward[i]            
    return scores, scores

@register_adv_est(AdvantageEstimator.PR_BN)
def compute_rn_bn_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    """
    Compute advantage for Process Reward BN (PR_BN), normalized by batch mean.
    """
    with torch.no_grad():
        scores = token_level_rewards.clone()
        batch_mean_top = (scores * response_mask).sum(-1) / (response_mask.sum(-1) + 1e-8)
        scores = scores - batch_mean_top.mean()
    return scores, scores

@register_adv_est(AdvantageEstimator.PR_DELTA)
def compute_rn_delta_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    """
    Compute advantage for Process Reward Delta (PR_DELTA) - we already subtract the previous step 
    score from the token level rewards. So we just return the token level rewards as the advantage.
    """
    with torch.no_grad():
        scores = token_level_rewards.clone()
    return scores, scores

@register_adv_est(AdvantageEstimator.PR_BASIC)
def compute_rn_basic_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    """
    Compute advantage for Process Reward Basic (PR_BASIC) - we return the token level rewards as the advantage.
    """
    with torch.no_grad():
        scores = token_level_rewards.clone()
    return scores, scores

@register_adv_est(AdvantageEstimator.BASIC)
def compute_basic_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    """
    Compute advantage for Basic (BASIC) - we return the token level rewards as the advantage.
    """
    with torch.no_grad():
        scores = token_level_rewards.clone()
...
@register_adv_est(AdvantageEstimator.GPPO)  # or simply: @register_adv_est("grpo")
def compute_gppo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    """
    Compute advantage for Process reward, where each response token has a reward.
    ...
    id2mask = defaultdict(list)    # number of tokens per score 
    id2mean = {}
    # id2std = {}
    scores = token_level_rewards.clone()
    # @TODO: temporarily only use mean 
    with torch.no_grad():
        bsz = scores.shape[0]
        for i in range(bsz):            
        ...
        for idx in id2score:
            ...
            mask_tensor = torch.stack(id2mask[idx])
            idx_scores = []
            for j in range(scores_tensor.shape[0]):
                idx_score = (scores_tensor[j] * mask_tensor[j]) / (mask_tensor[j].sum() + 1e-8)
                idx_scores.append(idx_score)
            id2mean[idx] = torch.stack(idx_scores).mean()
        for i in range(bsz):
...
def compute_policy_loss_reinforce(
    ...
    """Compute the QED-style REINFORCE objective with a detached importance-ratio cap."""

    assert config is not None
    assert not isinstance(config, AlgoConfig)

    reinforce_epsilon = float(config.policy_loss.get("reinforce_epsilon", 4.0))
    negative_approx_kl = torch.clamp(log_prob - old_log_prob, min=-20.0, max=20.0)
    ratio = torch.exp(negative_approx_kl)
    clipped_ratio = torch.clamp(ratio, min=0.0, max=1.0 + reinforce_epsilon)

    pg_losses = -(advantages * log_prob * clipped_ratio.detach())
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
    pg_clipfrac = verl_F.masked_mean(torch.gt(ratio, 1.0 + reinforce_epsilon).float(), response_mask)
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, response_mask)
...
@register_policy_loss("distill")
def compute_policy_loss_distill(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,    
    ...
    assert config is not None
    assert not isinstance(config, AlgoConfig)
    distill_loss = advantages * log_prob
    distill_loss = agg_loss(loss_mat=distill_loss, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
...
@register_policy_loss("vi_vanilla")
def compute_policy_loss_vi_vanilla(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    pg_loss_mask: torch.Tensor,
    ...
    Args:
        ...
        response_mask (torch.Tensor):
            Mask indicating which tokens to include in the loss, shape (batch_size, response_length).
        pg_loss_mask (torch.Tensor):
            Mask indicating which tokens to include in the policy gradient loss, shape (batch_size, response_length).
        sft_loss_mask (torch.Tensor):
            Mask indicating which tokens to include in the supervised fine-tuning loss, shape (batch_size, response_length).
        loss_agg_mode (str, optional):
            Aggregation mode for `agg_loss`. Defaults to "token-mean".
        rewards (torch.Tensor):
            Rewards for each action, shape (batch_size, response_length).
    """
    ...
    negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=20.0)
    ratio = torch.exp(negative_approx_kl)    
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, pg_loss_mask)
    ...
    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses1).float(), pg_loss_mask)

    pg_losses3 = -advantages * clip_ratio_c
    clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1)
    pg_clipfrac_lower = verl_F.masked_mean(
        torch.gt(clip_pg_losses1, pg_losses3) * (advantages < 0).float(), pg_loss_mask
    )
    ...
    ##### Compute SFT loss #####
    # convert outcome rewards to a single scaler for each question
    rewards = rewards.sum(dim=-1).unsqueeze(-1)
    sft_losses = -log_prob * rewards * sft_loss_mask * config.policy_loss.sft_loss_weight
